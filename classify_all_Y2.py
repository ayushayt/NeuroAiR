...
for subj in range(1,11):
    ...
    for modelname in ['EEGNet','DeepConvNet','ShallowConvNet']:
        ...
        for filename in ['EEG_Combined.txt']:
            ...
            X_train = np.zeros((2600,9,diff,1))
            ...
            for train_index,test_index in kf.split(X_train):
                ...
                if modelname=='EEGNet':
                    model = EEGNet(nb_classes=26,Chans=9,Samples=diff)
                elif modelname=='DeepConvNet':
                    model = DeepConvNet(nb_classes=26,Chans=9,Samples=diff)
                if modelname=='ShallowConvNet':
                    model = ShallowConvNet(nb_classes=26,Chans=9,Samples=diff)
    ...
    df.to_csv(subj_folder+'/results_Y2.csv',index=False)  

...
for subj in range(1,11):
    ...
    for modelname in ['EEGNet','DeepConvNet','ShallowConvNet']:
        ...
        for filename in ['EEG_Combined.txt']:
            ...
            eeg = eeg.to_numpy()
            print(eeg.shape)
            eeg = np.transpose(np.matmul(H,np.transpose(eeg)))
            print(eeg.shape)
            ...
            for i in range(0,len(event_arr)):
                ...
                if event_arr[i]!='PP':
                    ...
                    trial = int(i//52)
                    np.save('Segmented2/EEG_'+event_arr[i]+'_'+str(trial)+'.npy',eeg_sig)
            ...
            X_train = np.zeros((2600,16,diff,1))
            Y_train = np.zeros((2600,))
            
            trainctr = 0
                      
            for fi in os.listdir('Segmented2/'):
                eeg_sig = np.load(os.path.join('Segmented2/',fi))
                eeg_sig_var = np.zeros((diff,16))
            ...
            for train_index,test_index in kf.split(X_train):
                ...
                if modelname=='EEGNet':
                    model = EEGNet(nb_classes=26,Chans=16,Samples=diff)
                elif modelname=='DeepConvNet':
                    model = DeepConvNet(nb_classes=26,Chans=16,Samples=diff)
                if modelname=='ShallowConvNet':
                    model = ShallowConvNet(nb_classes=26,Chans=16,Samples=diff)
    ...
    df.to_csv(subj_folder+'/results_Y3.csv',index=False)  
